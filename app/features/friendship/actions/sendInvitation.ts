...
export async function sendInvitation(prevState: PrevState, formData: FormData) {
  ...
  const { data: friend } = await supabase.from("profiles").select("id").eq("user_email", validatedEmail);
  ...
  const { error } = await supabase.from("friends").insert([{ user_id: user?.id, friend_id: friend[0].id }]);
