...
export async function fetchTasks() {
  ...
  const { data: tasks } = await supabase
    ...
    .returns<Task[]>();
  const myTasks = tasks?.filter((task) => task.created_by === session?.user.id && task.assignee === session?.user.id);
  const sharedByOthers = tasks?.filter(
    (task) => task.created_by === session?.user.id && task.assignee !== session?.user.id,
  );
