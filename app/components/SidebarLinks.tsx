    ...
    <Accordion>
      <AccordionItem key="my-tasks" aria-label="My tasks" title="My tasks">
        <div className="flex flex-col gap-3">
          {categorizedTasks.myTasks.tasks.map(({ search, name, sizeOfTasks }) => (
            <Link
              key={search}
              className={cn(
                "text-black hover:bg-black hover:text-white   w-full text-lg text-start p-2 transition-all relative flex items-center",
                {
                  "bg-black text-white": searchQuery === search && categoryQuery === "myTasks",
                },
              )}
              href={{
                pathname: "/dashboard",
      ...
      <AccordionItem key="shared-by-others" aria-label="Shared by others" title="Shared by others">
        <div className="flex flex-col gap-3">
          {categorizedTasks.sharedByOthers.tasks.map(({ search, name, sizeOfTasks }) => (
            <Link
              key={search}
              className={cn(
                "text-black hover:bg-black hover:text-white  w-full text-lg text-start p-2 transition-all relative flex items-center",
                {
                  "bg-black text-white": searchQuery === search && categoryQuery === "sharedByOthers",
                },
              )}
              href={{
                pathname: "/dashboard",
      ...
      <AccordionItem key="shared-with-me" aria-label="Shared with me" title="Shared with me">
        <div className="flex flex-col gap-3">
          {categorizedTasks.sharedWithMe.tasks.map(({ search, name, sizeOfTasks }) => (
            <Link
              key={search}
              className={cn(
                "text-black hover:bg-black hover:text-white  w-full text-lg text-start p-2 transition-all relative flex items-center",
                {
                  "bg-black text-white": searchQuery === search && categoryQuery === "sharedWithMe",
                },
              )}
              href={{
                pathname: "/dashboard",
